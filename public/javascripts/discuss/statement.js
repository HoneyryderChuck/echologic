(function($) {
  $.widget("echo.statement", {
    ...
    _create: function() {
      ...
      that.statementType = that.domId.match("new") ? $.trim(statement.find('input#type').val()) :
                             $.trim(that.domId.match(/[^(add_|new_)]\w+[^_\d+]/)[0]);
    ...
    _insertStatement: function() {
      var that = this;
      if (!that.options.insertStatement) {
        return;
      }
      var statement = that.element;
      ...
      if (element.length > 0) {
        ...
        // No statement on this level of the stack, so insert at the bottom
    ...
    _collapseStatements: function() {
      ...
      $('#statements .statement .content').
        animate(that.options.hide_animation_params, that.options.hide_animation_speed);
      $('#statements .statement .header .supporters_label').
        animate(that.options.hide_animation_params, that.options.hide_animation_speed);
    },
    ...
    _initNavigationButton: function(button, inc) {
      ...
      var targetIndex = ($.inArray(currentStatementId, that.statementSiblings) + inc +
                          that.statementSiblings.length) % that.statementSiblings.length;
    ...
    // Returns an array of statement ids that should be loaded to the stack after a
    // link was clicked (and a new statement is loaded).
    ...
    _getTargetBids: function(key) {
      var that = this,
          ...
          index = $.inArray(key, targetBids);

      if (index != -1) {
        // If parent breadcrumb exists, then delete everything after it
        targetBids = targetBids.splice(0, index + 1);
      } else {
        // If parent breadcrumb doesn't exist, it means top stack statement
        ...
        if (originBids.length > 0) {
          ...
          index = $.inArray(lastOriginBid, currentBids);
          targetBids = targetBids.splice(0, index + 1);
        } else {
          // If it doesn't, it means there should be no breadcrumbs
          targetBids = [];
        }
      }
      return targetBids;
    ...
    _initStatementLinks: function(container, newLevel) {
      ...
      container.find('a.statement_link:not(.registered)').each(function() {
        $(this).bind("click", function(e) {
          ...
          altStack = altStack.length > 0 ? altStack.split(',') : [];
          var parentKey = that._getParentKey();
          ...
          // Insert new breadcrumb if this link triggers a new level in the stack
          if (newLevel) {
    ...
    _embedlyEmbed: function(oembed, dict) {
      ...
      if (oembed.type != 'link') {
        ...
        that._manualEmbed(that.embedPlaceholder, null);
    },
    ...
    _initContentLinks: function() {
      ...
      that.element.find(".statement_content a:not(.upload_link)").each(function() {
        ...
        // If this link goes to another echo statement => add a jump bid
        if (that._isEchoStatementUrl(url)) {
    ...
    _getParentKey: function() {
      ...
      if (that.parentStatement.length > 0) {
        if (statement.hasClass('alternative')) {
          // Get all alternative breadcrumbs
          // TODO: function from breadcrumbs api that returns alternative breadcrumbs
    ...
    _initChildrenScroll: function(container) {
      var that = this,
          maxPage = that.options.maxPage,
          maxHeight = that.options.maxHeight,
          elemHeight = that.options.childHeight,
          children = container;

      if (!container.hasClass('children_container')) {
        children = children.find('.children_container');
      }
      ...
      if (children.hasClass('double')) {
        maxPage = maxPage.dual;
        maxHeight = maxHeight.dual;
        elemHeight = elemHeight.dual;
      } else {
    ...
    _reinitialiseChildren: function(container) {
      ...
      if (that.isEchoable) {
        st.data('echoable').loadRatioBars(container);
      }
    },
    ...
    insertChildren: function(type, children, page) {
      ...
      if (container.length > 0) {
        ...
        container.parent().hasClass('siblings_panel') ?
          that._reinitialiseSiblings(container) : that._reinitialiseChildren(container);
      } else {
    ...
    loadAuthors: function(authors) {
      var that = this,
          statement = that.element,
          length = authors.find('.author').length;
    ...
    historyStateUrl: function() {
      ...
      var params = $.extend({}, echoApp.hash_state),
