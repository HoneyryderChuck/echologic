(function($) {
  ...
  $.widget("echo.taggable", {
    ...
    _loadTags: function(tags) {
      var tagsToLoad = $.trim(tags).split(','),
          i;
    ...
    _loadTagEvents: function() {
      ...
      that.tagInput.bind('keypress', (function(e) {
        ...
        if (e && e.keyCode == $.ui.keyCode.ENTER) {
      ...
      that.addButton.bind('click', (function(e) {
        ...
        if (enteredTags.length != 0) {
          ...
          /* Trimming all tags */
          enteredTags = $.map(enteredTags, function(tag) {
            return $.trim(tag);
          });
          ...
          existingTags = $.map(existingTags, function(q) {
            return $.trim(q);
          });
    ...
    _createTagButton: function(text) {
      ...
      deleteButton.bind('click', function(e) {
        ...
        formTags = $.map(formTags, function(q) {
          return $.trim(q)
        });
    ...
    addTags: function(tags) {
      var that = this,
          loadedTags = $.trim(that.loadedTags.val());
