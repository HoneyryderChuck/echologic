(function($) {
  ...
  $.widget("echo.breadcrumbs", {
    ...
    _initBreadcrumb: function(breadcrumb) {
      if (!breadcrumb.children(':last').hasClass('statement')) {
        return;
      }
      var that = this;
    ...
    _cleanBreadcrumbs: function() {
      ...
      if (deleteFrom && deleteFrom.length > 0) {
        // If a special link was clicked
        if ($.inArray(deleteFrom.substring(0, 2), ['ds','sr']) != -1) {
          deleteFrom = deleteFrom.substring(0, 2);
        }

        // Get breadcrumbs ordered per id
        var breadcrumbIds = that.container.find('.breadcrumb').map(function() {
          return $(this).attr('id');
        });
        ...
        // Delete all breadcrumbs that are not in the state bids
    ...
    // Removes the breadcrumb ONLY VISUALLY
    deleteBreadcrumb: function(key) {
      var that = this;
      if (key && key.length > 0) {
    ...
    breadcrumbsToLoad : function (bids) {
      ...
      if (deleteFrom) {
        ...
        var visibleBids = breadcrumbs.find(".breadcrumb").map(
                function() {
                  return that._truncateBreadcrumbKey($(this));
                }).get();
    ...
    getBreadcrumbStack : function (newBreadcrumb) {
      var that = this;
      var currentBreadcrumbs = that.container.find(".breadcrumb").map(
              function() {
                return that._truncateBreadcrumbKey($(this));
              }).get();
      if (newBreadcrumb)
